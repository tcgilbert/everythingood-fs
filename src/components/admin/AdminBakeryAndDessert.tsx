...
interface AdminEntryProps {
  data: BakeryAndDessert;
  refetch: () => void;
}

export const AdminBakeryAndDessert = ({ data, refetch }: AdminEntryProps) => {
  const [open, setOpen] = useState(false);
  const [modalInputs, setModalInputs] = useState({ ...data });
  const { name } = data;
  const { mutateAsync: updateEntry } =
    api.guide.patchBakeryAndDessert.useMutation();
  ...
  function RenderObject(entry: BakeryAndDessert) {
    const movePropertiesToEnd = (obj: BakeryAndDessert) => {

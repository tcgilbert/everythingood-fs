...
export const AdminRestaurant = ({ data, refetch }: AdminEntryProps) => {
  const [open, setOpen] = useState(false);
  const [modalInputs, setModalInputs] = useState({ ...data });
  const { name } = data;
  console.log(data);
  const { mutateAsync: updateEntry } = api.guide.patchRestaurant.useMutation();
        ...
        return (
          <div key={key} className="flex flex-col py-1">
            ...
            <input
              ...
              onChange={(e) => {
                setModalInputs({ ...modalInputs, idealMeal: e.target.value });
              }}
            />
          </div>
        );
        ...
        return (
          <div key={key} className="col-span-2 flex flex-col py-1">
            ...
            <textarea
              ...
              onChange={(e) => {
                setModalInputs({ ...modalInputs, alsoGreat: e.target.value });
              }}
            />
          </div>
        );
        ...
        return (
          <div key={key} className="flex flex-col py-1">
            ...
            <input
              ...
              onChange={(e) => {
                setModalInputs({ ...modalInputs, drinkOrder: e.target.value });
              }}
            />
          </div>
        );
  ...
  function RenderObject(entry: Restaurant) {
    const movePropertiesToEnd = (obj: Restaurant) => {
      const { description, alsoGreat, note, ...rest } = obj;
      return { ...rest, description, alsoGreat, note };
    };

...
interface AdminEntryProps {
  data: GroceryAndLiquor;
  refetch: () => void;
}

export const AdminGroceryAndLiquor = ({ data, refetch }: AdminEntryProps) => {
  const [open, setOpen] = useState(false);
  const [modalInputs, setModalInputs] = useState({ ...data });
  const { name } = data;
  const { mutateAsync: updateEntry } =
    api.guide.patchGroceryAndLiquor.useMutation();
  ...
  function RenderObject(entry: GroceryAndLiquor) {
    const movePropertiesToEnd = (obj: GroceryAndLiquor) => {

...
export const AdminCafe = ({ data, refetch }: AdminEntryProps) => {
  ...
  function handleModalInputs(key: string, value: string) {
    switch (key) {
        ...
        return (
          <div key={key} className="flex flex-col py-1">
            ...
            <input
              ...
              onChange={(e) => {
                setModalInputs({ ...modalInputs, bakedGoods: e.target.value });
              }}
            />
          </div>
        );
      ...
      case "coffeeBeans":
        return (
          <div key={key} className="coflex flex-col py-1">
            <label className="block text-sm font-medium text-gray-700">
              Coffee Beans:
            </label>
            <input
  ...
  function RenderObject(entry: Cafe) {
    const movePropertiesToEnd = (obj: Cafe) => {
